...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = (
            User.objects(email=form.email_or_username.data).first()
            or User.objects(username=form.email_or_username.data).first()
        )
        ...
        if user:
            try:
                ...
                flash(
                    "Error al verificar la contraseña. Por favor, intenta restablecer tu contraseña.",
                    "danger",
                )
                logger.error(
                    f"Error al verificar la contraseña para el usuario {user.username}: {e}"
                )
        else:
