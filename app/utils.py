...
def create_default_image() -> None:
    ...
    default_image_path = (
        Path(current_app.root_path) / "static" / "container_pics" / "default.png"
    )
...
def generate_qr(
    data: str, filename: Union[str, Path], output_size: Tuple[int, int] = (300, 300)
) -> bool:
    """
...
def save_qr_image(data: str, container_name: str) -> Path:
    ...
    """
    safe_container_name = (
        re.sub(r"[^\w\s-]", "", container_name).strip().replace(" ", "_")
    )
...
"""
    try:
        ...
        current_app.logger.info(
            f"Correo de restablecimiento de contraseña enviado a {user.email}"
        )
    except Exception as e:
        current_app.logger.error(
            f"Error al enviar correo de restablecimiento: {str(e)}"
        )
