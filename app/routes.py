...
def account():
    ...
    if form.validate_on_submit():
        try:
            ...
            current_app.logger.info(
                f"Iniciando actualización de cuenta para el usuario: {current_user.username}"
            )
            current_app.logger.info(
                f"Información recibida - Username: {form.username.data}, Email: {form.email.data}, "
            ...
            if form.picture.data:
                current_app.logger.info(
                    f"Actualizando foto de perfil para el usuario: {current_user.username}"
                )
                try:
                    picture_file = save_profile_picture(form.picture.data)
                    current_user.image_file = picture_file
                    current_app.logger.info(
                        f"Foto de perfil actualizada: {picture_file}"
                    )
                except Exception as e:
                    current_app.logger.error(
                        f"Error al actualizar la foto de perfil para {current_user.username}: {e}"
                    )
                    flash(
            ...
            if form.username.data != current_user.username:
                current_app.logger.info(
                    f"El nombre de usuario ha cambiado de {current_user.username} a {form.username.data}"
                )
            ...
            current_user.save()
            current_app.logger.info(
                f"Cuenta actualizada exitosamente para el usuario: {current_user.username}"
            )
            ...
            current_app.logger.error(
                f"Error actualizando la cuenta para el usuario {current_user.username}: {e}"
            )
            flash(
                "Ocurrió un error al actualizar la cuenta. Intenta de nuevo.", "danger"
            )
    ...
    current_app.logger.info(
        f"Mostrando imagen de perfil para el usuario {current_user.username}: {current_user.image_file}"
    )

    return render_template(
        "account.html", title="Cuenta", image_file=image_file, form=form
    )
