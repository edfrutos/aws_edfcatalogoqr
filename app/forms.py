...
class LoginForm(FlaskForm):
    ...
    email_or_username = StringField(
        "Email o Nombre de Usuario", validators=[DataRequired()]
    )
...
class ContainerForm(FlaskForm):
    ...
    name = StringField(
        "Nombre del Contenedor", validators=[DataRequired(), Length(min=2, max=50)]
    )
    location = StringField(
        "Ubicación", validators=[DataRequired(), Length(min=2, max=100)]
    )
    items = TextAreaField(
        "Elementos (separados por comas)", validators=[DataRequired(), Length(max=1000)]
    )
    ...
    def validate_name(self, name):
        ...
        if container:
            raise ValidationError(
                "Ya tienes un contenedor con este nombre. Por favor, elige otro."
            )
...
class EditContainerForm(FlaskForm):
    ...
    name = StringField(
        ...
        validators=[
            DataRequired(),
            Length(
                min=2, max=50, message="El nombre debe tener entre 2 y 50 caracteres."
            ),
        ],
    )
    location = StringField(
    ...
    items = TextAreaField(
        ...
        validators=[
            ...
            Length(
                max=1000,
                message="La lista de objetos no puede exceder los 1000 caracteres.",
            ),
        ],
    )
    pictures = MultipleFileField(
        ...
        validators=[
            Optional(),
            FileAllowed(
                ALLOWED_IMAGE_EXTENSIONS, "Solo se permiten imágenes (jpg, jpeg, png)."
            ),
            FileSize(
                max_size=MAX_FILE_SIZE, message="El tamaño máximo por imagen es 5MB."
            ),
        ],
    )
    ...
    def validate_name(self, field):
        ...
        if self.original_container and field.data != self.original_container.name:
            container = Container.objects(
                name=field.data, user=current_user.id, id__ne=self.original_container.id
            ).first()
...
class ResetPasswordForm(FlaskForm):
    password = PasswordField(
        "Contraseña", validators=[DataRequired(), Length(min=6, max=20)]
    )
    confirm_password = PasswordField(
...
class ContactForm(FlaskForm):
    ...
    message = TextAreaField(
        "Mensaje", validators=[DataRequired(), Length(min=10, max=1000)]
    )
...
class SearchUserForm(SearchForm):
    ...
    search_query = StringField(
        "Buscar Usuario (por nombre de usuario o email)", validators=[DataRequired()]
    )
    submit = SubmitField("Buscar")
...
class ChangePasswordForm(FlaskForm):
    ...
    current_password = PasswordField(
        "Contraseña actual",
        validators=[DataRequired(message="Por favor, introduce tu contraseña actual.")],
    )
