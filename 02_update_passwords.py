...
class User(db.Document):
    ...
    def set_password(self, password):
        ...
        self.password = generate_password_hash(password, method='pbkdf2:sha256', salt_length=8).decode('utf-8')
    ...
    def save(self, *args, **kwargs):
        if self.password and not self.password.startswith('pbkdf2:sha256'):
            ...
            self.password = generate_password_hash(self.password, method='pbkdf2:sha256', salt_length=8).decode('utf-8')
        ...
        for user in users:
            ...
            if user.password and not user.password.startswith('pbkdf2:sha256'):
                ...
                if user.password != original_password:
                    ...
                    print(f"Error al actualizar la contraseña para el usuario {user.username}")
